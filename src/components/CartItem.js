...
function CartItem({ item, add, remove }) {
    ...
    var addToCart = () => { setNumber(++number); add(item.price); };
    var addItem = () => { setNumber(++number); add(item.price); };
    var removeItem = () => { if (number > 0) { setNumber(--number); remove(item.price); } };
    return (<div className="cart-item">
        ...
        <div className="item-details">
            ...
            <div className="actions">
                ...
                <AddRemoveQuantity quantity={number} addOne={addItem} removeOne={removeItem} />
            </div>
        </div>
    </div >)
}
